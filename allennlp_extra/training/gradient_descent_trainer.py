...
@Trainer.register("my_gradient_descent", constructor="from_partial_objects")
class GradientDescentTrainer(Trainer):
    """ A copy of `allennlp.training.GradientDescentTrainer` with extra functions
    1. evaluate with test set at the end of each epoch
    2. `get_test_metric_for_each_update`: option to evaluate with test set at each batch update 
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._test_data_loader is not None:
            self._test_data_loader.set_target_device(self.cuda_device)
        ...
        for batch_group in batch_group_generator_tqdm:
            ...
            if self.update_test_metric and self._total_batches_completed % self.update_test_metric == 1 and self._test_data_loader is not None:
                with torch.no_grad():
                    ...
                    try:
                        train_metric = deepcopy(self.model._accuracy)
                    except:
                        raise Exception('The metric in the model does not support `get_test_metric_for_each_update`')
                    self.model.get_metrics(reset=True)
                    for batch in test_generator_tqdm:
                        batch_outputs = self.model(**batch)
                    test_metrics = self.model.get_metrics(reset=True)
                    
                    for callback in self._callbacks:
                        if isinstance(callback, WandBCallback):
                            callback.wandb.log(
                                {"test_accuracy": test_metrics['accuracy'], 
                                 "train_accuracy": train_metric.get_metric(),
                                "epoch": epoch}, 
                                step=self._total_batches_completed,
                            )

                            break
                    self.model.train()
                    self.model._accuracy = train_metric
        ...
        if self._test_data_loader is not None:
                with torch.no_grad():
                    ...
                    for key, value in test_metrics.items():
                        metrics["untrained_test_" + key] = value
            ...
            if self._test_data_loader is not None:
                with torch.no_grad():
                    ...
                    for key, value in test_metrics.items():
                        ...
                        for callback in self._callbacks:
                            if callback.__class__ == WandBCallback:
                                callback.log_scalars(
                                    {key: value ,},
                                    log_prefix="test",
                                    epoch=epoch,
                                )
    ...
    # ============= my comment ==========
    # it is import to have `from_partial_objects` to  construct Lazy objects for 
    # optimizer(need parameters), learning_rate_scheduler(need optimizer), 
    # momentum_scheduler(need optimizer), moving_average(need parameters)
    # For example,if we set :Optimizer instead of Lazy[Optimizer], it will get the orig dict
    # 
    # ===================================
    @classmethod
