# re-implement SimpleSeq2Seq with 
# (1) transformer tokenization/indexing
# (2) attention
import warnings
...
class MySeq2Seq(Model):
    ...
    """" Refer to `from allennlp_models.generation.models import SimpleSeq2Seq`, but
    (1) a shared embedding layer for the encoder and decoder, i.e., target_namespace is also for input
    (2) compitible to input from PretrainedTransformerTokenizer and retrainedTransformerIndexer
        by specifying `indexer_output_key` to `token_ids`
    (3) set up attention module if `attention is True`
    (4) add ROUGE scores
    """
    def __init__(
        self,
        ...
        **kwargs
        ...
        ##### the following five lines are different from `SimpleSeq2Seq.__init__()`
        self.indexer_output_key = indexer_output_key
        if indexer_output_key == "token_ids":
            config = PretrainedConfig.from_pretrained(model_name)
            self._start_index = config.bos_token_id  # CLS
            self._end_index = config.eos_token_id  # SEP
            pad_index = config.pad_token_id  # PAD
        else:
        ...
        self._target_embedder = self._source_embedder.token_embedder_tokens
        ...
        if attention:
            ...
            self._attention = AdditiveAttention(matrix_dim=self._encoder_output_dim, vector_dim=self._decoder_output_dim)

            # If using attention, a weighted average over encoder outputs will be concatenated
            # to the previous target embedding to form the input to the decoder at each
            # time step.
            self._decoder_input_dim = self._decoder_output_dim + source_embedder.get_output_dim()
        else:
            # Otherwise, the input to the decoder is just the previous target embedding.
            self._decoder_input_dim = source_embedder.get_output_dim()
    ...
    def forward(
        ...
        """
        source_mask=None
        if self.indexer_output_key == "token_ids":
            # unlike transformer models, `source_embedder.token_embedder_tokens` only needs `tokens` tensor
            source_tokens['tokens']['tokens'] = source_tokens['tokens'].pop('token_ids') 
            source_mask = source_tokens['tokens'].pop('mask')
            source_tokens['tokens'].pop('type_ids')
        state = self._encode(source_tokens, source_mask) # state['encoder_outputs'].shape: (bsz, timesteps, hsz) # state['source_mask'].shape: (bsz, timesteps)
        ...
        if not self.training:
            ...
            if target_tokens and self._bleu:
                ...
                self._bleu(best_predictions, target_tokens["tokens"][self.indexer_output_key])
            self._rouge(best_predictions, target_tokens["tokens"][self.indexer_output_key])
        return output_dict
    ...
    def _encode(self, source_tokens: Dict[str, torch.Tensor], source_mask=None) -> Dict[str, torch.Tensor]:
    ...
    def _forward_loop(
        ...
        if target_tokens:
            ...
            targets = target_tokens["tokens"][self.indexer_output_key]
    ...
    def _prepare_output_projections(
        ...
        if self._target_decoder_layers > 1:
            ...
            _, (decoder_hidden, decoder_context) = self._decoder_cell(
                decoder_input.float(), (decoder_hidden.float().contiguous(), decoder_context.float().contiguous())
            )
        else:
